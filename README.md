...
| `stats/entropy/OG_genes_entropy.csv` | OG-to-gene mapping table generated from `stats/OG_genes.tsv` for the entropy workflow in step 3. |
...
| `--add_domain` | Optional domain CSV (`gene,domain,start,end`) to add domain ranges to per-gene entropy plots. The `gene` value must match the `gene` column in `stats/entropy/OG_genes_entropy.csv` generated in step 1. |
| `-h, --help` | Show help. |
